...
bool TrackballCameraController::update(float elapsedTime) 
{
 ...
 // Rotate around target
  ...
  const glm::vec3 depthAxis = m_camera.getEye() - camera.getCenter();
  ...
  const glm::vec3 newEye = m_camera.getCenter() + rotatedDepthAxis;

  m_camera = Camera(newEye, m_camera.getCenter(), m_worldUpAxis);
