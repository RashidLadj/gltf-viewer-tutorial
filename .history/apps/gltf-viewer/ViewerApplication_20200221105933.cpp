...
int ViewerApplication::run()
{
  ...
  const auto drawScene = [&](const Camera &camera) {
    ...
    const std::function<void(int, const glm::mat4 &)> drawNode = [&](int nodeIdx, const glm::mat4 &parentMatrix) {
          ...
          if (node.mesh >= 0){
            const glm::mat4 modelViewMatrix = getLocalToWorldMatrix(node, parentMatrix);
            const glm::mat4 modelViewProjectionMatrix = getLocalToWorldMatrix(node, parentMatrix);
            const glm::mat4 normalMatrix  = getLocalToWorldMatrix(node, parentMatrix);
